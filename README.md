# E-Learning-Platform

https://platforma.foreverlearning.ro

A modern full-stack e-learning platform designed to manage online courses, virtual classrooms, and student progress in one unified system. The app allows professors to create and organize courses, upload learning materials, assign homework, and host live video sessions, while students can enroll in courses, attend classes, submit assignments, and track their learning progress.
<br>

Key functionalities include secure authentication with role-based access (student/professor), automatic user provisioning from Auth to the public users table, course and enrollment management, announcements and attachments, assignment submission workflows, real-time video rooms powered by Daily.co, and a responsive React interface.
